...
<style>
    ...
    .stButton > button {
        ...
        width: 100% !important; box-shadow: 0 4px 6px rgba(0,0,0,0.1) !important;
    }
    ...
    /* =========================================
       ✨ 모바일 네이티브 테이블 커스텀 CSS ✨
       ========================================= */
    .table-container {
        width: 100%;
        overflow-x: auto;
        -webkit-overflow-scrolling: touch; /* 부드러운 네이티브 스크롤 복구 */
        border-radius: 10px;
    ...
    .custom-table {
        ...
        font-size: 0.85rem; /* 폰트를 줄여 타이트하게 화면에 맞춤 */
        text-align: center;
    }
    .custom-table th {
        background-color: #F8F9FA; /* 헤더 연한 회색 */
        color: #555555;
        font-weight: 700;
        padding: 8px 4px; /* 여백을 줄여 공간 확보 */
        border-bottom: 2px solid #EAEAEA;
        white-space: nowrap;
    }
    .custom-table td {
        padding: 10px 4px; /* 여백 타이트하게 */
...
with tab1:
    if curr_df.empty: st.info("조건에 부합하는 종목이 없습니다.")
        ...
        # 데이터 포맷팅 (칼럼명 압축 & $ 기호 추가)
        display_df = curr_df.copy()
        display_df.rename(columns={'시그널 발생일자': '발생일', 'Ticker': '종목', 'Close': '현재가', '눌림목 점수': '점수'}, inplace=True)
        ...
        # HTML로 렌더링
        html_table = display_df.to_html(index=False, classes="custom-table")
...
with tab2:
    if bt_df.empty: st.info("1달 전 조건에 부합했던 종목이 없습니다.")
        ...
        display_bt.rename(columns={'시그널 발생일자': '발생일', 'Ticker': '종목', '눌림목 점수': '점수', 
                                   '발생일 종가': '당시가', '이후 최고가': '최고가', '최대 수익률(%)': '수익률'}, inplace=True)
        display_bt['당시가'] = display_bt['당시가'].apply(lambda x: f"${x:,.2f}")
        display_bt['최고가'] = display_bt['최고가'].apply(lambda x: f"${x:,.2f}")
        display_bt['수익률'] = display_bt['수익률'].apply(lambda x: f"{x:,.2f}%")
        
        html_bt_table = display_bt.to_html(index=False, classes="custom-table")
        st.markdown(f'<div class="table-container">{html_bt_table}</div>', unsafe_allow_html=True)
