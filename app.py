        ...
        try:
            ...
            if cond1 and cond2 and cond3 and cond4 and cond5:
                ...
                if is_backtest:
                    ...
                    item.update({'최대 수익률(%)': round(max_return, 2)})
...
def fetch_data_and_analyze():
    ...
    # 2. 1-Month Backtest 분석 (API 최소 호출을 위해 이미 아는 섹터는 캐시 사용)
    ...
    bt_df = pd.DataFrame(bt_results).sort_values(by=['최대 수익률(%)', '시그널 발생일자'], ascending=[False, False]) if bt_results else pd.DataFrame()
        
    return curr_df, bt_df, datetime.now()

# =========================
# 3. 데이터 로드 & 자동 자가 치유(Self-Healing) 로직
# =========================
with st.spinner('Scanning Market Data...'):
    curr_df, bt_df, last_update = fetch_data_and_analyze()

# 💡 만약 야후 API 차단 등으로 둘 다 데이터가 비어있다면, 캐시를 스스로 초기화하고 한 번 더 안전하게 재시도
if curr_df.empty and bt_df.empty:
    fetch_data_and_analyze.clear()
    st.warning("⚠️ 야후 API 응답 지연으로 재시도 중입니다. 잠시만 기다려주세요...")
    st.rerun()
