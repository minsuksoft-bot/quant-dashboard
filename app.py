...
with st.sidebar:
    ...
    st.caption("외부 사용자는 캐시를 초기화할 수 없습니다.")
    admin_password = st.text_input("관리자 패스워드", type="password")
    
    # 비밀번호가 맞을 때만 초기화 권한 부여 (기본 비밀번호: quant1234)
    is_authenticated = (admin_password == "quant1234")
    
    if is_authenticated:
        st.success("관리자 권한 인증됨")
        if st.button("🚨 강제 캐시 초기화 및 재조회", use_container_width=True):
            st.cache_data.clear()
            st.rerun()
    else:
        if admin_password:
...
def fetch_data_and_analyze():
    ...
    nasdaq_tickers = [t for t in tickers_df['Symbol'].tolist()[:300] if is_valid_ticker(t)]
    ...
    return curr_df, bt_df, datetime.now(), downloaded_count

with st.spinner('Scanning Market Data...'):
    curr_df, bt_df, last_update, dl_count = fetch_data_and_analyze()

st.caption(f"🔄 Last Updated: {last_update.strftime('%m.%d %H:%M:%S')} | 수신 종목 수: **{dl_count}개**")
st.markdown("<br>", unsafe_allow_html=True)

tab1, tab2 = st.tabs(["LIVE SIGNALS", "1-MONTH TEST"])

with tab1:
    if curr_df.empty: st.info("현재 조건에 부합하는 종목이 없습니다. (야후 API 제한 상태일 수 있으니 잠시 후 관리자 메뉴에서 초기화해보세요)")
    else:
