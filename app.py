...
def run_dashboard_analysis():
    ...
    # 원본과 완벽히 동일하게 group_by 없이 다운로드하여 딕셔너리 구조 유지
    data = yf.download(nasdaq_tickers, period="6mo", threads=True, progress=False)

    downloaded_tickers = len(data.columns.levels[1]) if isinstance(data.columns, pd.MultiIndex) else len(data)
